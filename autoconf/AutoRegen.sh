...
autoconf_version=`autoconf --version | head -1 | grep -Eo '[0-9]+\.[0-9]+'`
autoconf_major_version=`echo "$autoconf_version" | sed -e 's/\..*//'`
autoconf_minor_version=`echo "$autoconf_version" | sed -e 's/[^.]*\.//' | sed -e 's/\..*//'`
if [ "$autoconf_major_version" -lt 2 -o "$autoconf_minor_version" -lt 60 ]; then
