/* this is meant to be stacked unlike 'objwide'; also tries harder to avoid
 * wide stores / loads. */
...
do {	uint64_t obj_sz;						\
	...
	if ((p_64 + sizeof(y)) <= (obj_base+obj_sz)) { 			\
		uint64_t	c2 = klee_get_value(p_64);		\
		if (klee_valid_eq(c2, p_64)) {				\
			klee_assume_eq(addr, c2);			\
			addr = (void*)c2;				\
		}							\
		found_op; }						\
...
#define MMU_LOAD(x,y)		\
y mmu_load_##x##_objwideS(void* addr) {	\
	MMU_FIND_PAGES			\
	MMU_ITER(y, return klee_wide_load_##x((void*)obj_base, addr));	\
	return *((y*)c_64); }

#define MMU_STORE(x,y)			\
void mmu_store_##x##_objwideS(void* addr, y v) {	\
	MMU_FIND_PAGES				\
