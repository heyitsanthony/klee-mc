...
static void memchr_enter(void* r)
{
	...
	klee_assume_uge(ret, i);
	klee_assume_ule(ret, clo.len);
	GET_SYSRET(r) = klee_mk_ite(
		klee_mk_eq(ret, clo.len),
		0,
		s + ret);
		
	clo.p = virtsym_safe_memcopy(clo.p);
...
static void memchr_fini(uint64_t _r, void* aux)
{
	...
	/* should not exist in string */
	if (klee_feasible_eq(_r, clo->len)) {
		for (i = 0; i < clo->len; i++) {
			if (!klee_feasible_ne(s[i], clo->c))
				break;
		}
