...
static int __is_string_symbolic(const char *s);
static const char *__concretize_string(const char *s);

static int __strcmp(const char* x, const char* y)
{
	unsigned i;
	for (i = 0; x[i] && x[i] == y[i]; i++);
	if (!x[i]) return 0;
	return x[i] - y[i];
}

/* check if we've seen this pathname before */
static exe_disk_file_t* __path_already_seen(const char* pathname) {
  unsigned i;

  for (i=0; i < __exe_fs.n_sym_files_used; i++) {
...
/* Tries to validate the given symbolic file name.  Ignores concrete
   chars. */
static void __validate_file_name(const char* pathname, int pathlen) {
  int i;

  for (i=0; i<pathlen; i++)
    if (klee_is_symbolic(pathname[i])) {
      if (pathname[i] == '/')
	klee_silent_exit(0);
    }
}
...
static exe_disk_file_t *__get_sym_file(const char *pathname)
{
  ...
  /* This essentially makes the file names as short as possible */
  prune_id = klee_get_prune_id(1);
  pathlen = strlen(pathname);
  klee_prune(prune_id);

  if (!__is_string_symbolic(pathname)) {
    /* for now, concrete paths cannot point to symbolic files, unless
       this concrete file was used as the name of a previously
       accessed symbolic file  */
    ...
    if (syscall(__NR_stat64, pathname, buf) == 0)
      return NULL;
#endif
  }
  ...
  /*pathname = __concretize_string(pathname);*/
  if (pathlen >= KLEE_MAX_PATH_LEN) {
    klee_warning("Pathname too large, terminating path.  Increase KLEE_MAX_PATH_LEN and rerun\n.");
    klee_silent_exit(1);
  }
  ...
  __validate_file_name(pathname, pathlen);
...
/* Returns pointer to the symbolic file structure if the pathname is symbolic,
   relative to file descriptor fd (see do_path_lookup() in linux kernel source
   file fs/namei.c) */
exe_disk_file_t* __get_fileat(int fd, const char *pathname) {
  if (pathname[0] == '/') {
    return __get_sym_file(pathname);
  } else if (fd == AT_FDCWD) {
    return __get_sym_file(pathname);
  } else {
    /* !!! TODO: implement actual path resolution */
    klee_warning("__get_fileat not fully implemented (EBADF)");
    errno = EBADF;
  }
  return 0;
}
...
/* Returns next available fd.  Sets eOpen in associated flags.
   If no more fds are available, returns -1 and sets errno to EMFILE.
...
void  __undo_get_new_fd(int fd) {
  ...
  if (f->refs == 0) {
    free(f);
    __exe_env.fds[fd] = NULL;
  }
}


static int setup_sym_f(exe_file_t *f, exe_disk_file_t *df, int flags, int mode)
{
    ...
    if (flags & O_DIRECTORY)
      f->dfile->stat->st_blksize = klee_get_value(f->dfile->stat->st_blksize);
...
int __fd_openat(int dirfd, const char *pathname, int flags, mode_t mode)  {
  ...
  df = __get_fileat(dirfd, pathname);
  if (df) {
    /* XXX Should check access against mode / stat / possible deletion. */
    f->dfile = df;
    ...
    if ((flags & O_TRUNC) && (flags & O_RDONLY)) {
      /* The result of using O_TRUNC with O_RDONLY is undefined, so we
         return error */
      fprintf(stderr, "Undefined call to openat(): O_TRUNC | O_RDONLY\n");
      errno = EACCES;
      return -1;
    }
    ...
    if ((flags & O_EXCL) && !(flags & O_CREAT)) {
      /* The result of using O_EXCL without O_CREAT is undefined, so
         we return error */
      fprintf(stderr, "Undefined call to openat(): O_EXCL w/o O_RDONLY\n");
      errno = EACCES;
      return -1;
    }
...
int close(int fd) {
  ...
  // sbkim: closing the concrete file will cause other paths to fail
#if 0
  if (!f->dfile) {
    /* if a concrete fd */
...
ssize_t __fd_scatter_read(exe_file_t *f, const struct iovec *iov, int iovcnt)
{
  ...
  for (i = 0; i < iovcnt; ++i) {
    size_t this_len = iov[i].iov_len;
    if (read_so_far + this_len > to_read)
      this_len = to_read - read_so_far;
    memcpy(iov->iov_base, f->dfile->contents + f->off, this_len);
    read_so_far += this_len;
    f->off += this_len;
  }
...
int __fd_statat(int dirfd, const char *path, struct stat64 *buf, int flags) {
  /* XXX pay attention to flags */
  exe_disk_file_t *dfile = __get_fileat(dirfd, path);
  if (dfile) {
    memcpy(buf, dfile->stat, sizeof(*dfile->stat));
    return 0;
  }
  ...
  {
#if __WORDSIZE == 64
    int r = syscall(__NR_newfstatat, dirfd, __concretize_string(path), buf,
      flags);
#else
...
int fchmodat(int fd, const char *file, mode_t mode, int flag) {
  ...
  exe_disk_file_t *dfile = __get_fileat(fd, file);

  n_calls++;
  if (__exe_fs.max_failures && *__exe_fs.fchmodat_fail == n_calls) {
    __exe_fs.max_failures--;
    errno = EIO;
    return -1;
  }
...
int ioctl(int fd, unsigned long request, ...) {
  ...
  if (f->dfile) {
    ...
    /*klee_make_symbolic(ia, sizeof(*ia), "IFADDR");*/
      memset(ia, strncmp(ifr->ifr_name, "lo", IFNAMSIZ) != 0 ? 0xdd : 0, sizeof(*ia));
      return 0;
    }
    case SIOCGIFADDR: {
      struct ifreq *ifr = buf;
...
int fcntl(int fd, int cmd, ...) {
  ...
  if (f->dfile) {
    ...
    case F_GETFL: {
      ...
      */
      klee_warning_once("returning O_RDWR for F_GETFL");
      return O_RDWR;
    }
    case F_SETFL: {
      /* XXX (CrC): This should set the status flags: O_APPEND,
	 O_ASYNC, O_DIRECT, O_NOATIME, O_NONBLOCK.
      */
      klee_warning_once("ignoring F_SETFL");
      return 0;
    }
    default:
      klee_warning("symbolic file, ignoring (EINVAL)");
...
int dup2(int oldfd, int newfd)
{
	...
	f2 = __exe_env.fds[newfd];
	if (f2 && (f2->flags & eOpen)) close(newfd);

	__exe_env.fds[newfd] = f;
	++f->refs;
...
int select(int nfds, fd_set *read, fd_set *write,
  ...
  if (__exe_fs.max_failures && *__exe_fs.select_fail == n_calls) {
    __exe_fs.max_failures--;
    errno = EIO;
    return -1;
  }
...
static void *__concretize_ptr(const void *p) {
#if __WORDSIZE == 64
  void *pc = (void*) klee_get_value((const uint64_t) p);
#else
...
/* It's important for this not to fork */
static int __is_string_symbolic(const char *s) {
  int prune1 = klee_get_prune_id(1);
  int prune2 = klee_get_prune_id(1);

  while (!klee_is_symbolic(*s) && (*s != '\0')) {
    /*fprintf(stderr, "HERE\n");*/
    s++;
  }

  if (klee_is_symbolic(*s)) {
    if (*s == '\0')
      klee_silent_exit(0);
    klee_prune(prune1);
    return 1;
