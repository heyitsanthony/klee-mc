// RUN: gcc %s -O0 -o %t1
// RUN: rm -rf guest-vmemchr
// RUN: ../../../scripts/save_guest.sh ./%t1 guest-vmemchr
// RUN: klee-mc -stop-after-n-tests=100 -use-hookpass -guest-sshot=guest-vmemchr -guest-type=sshot -hookpass-lib=libvirtsyms.bc - ./%t1 2>%t1.err >%t1.out
...
typedef char*	(*sc_f)(const char* s, char c, size_t sz);
