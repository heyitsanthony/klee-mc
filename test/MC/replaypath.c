...
// RUN: klee-mc -seed-rng=10 -pipe-solver -chk-constraints -exit-on-error -mm-type=deterministic -replay-path-dir=klee-out-replay/ -stop-after-n-tests=21 -guest-type=sshot -guest-sshot=guest-replay - 2>>%t1.2.err >>%t1.out
