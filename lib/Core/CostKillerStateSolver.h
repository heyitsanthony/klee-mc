...
public:
	...
	bool mustBeTrue(const ExecutionState& es,
			ref<Expr> e,
	...
	bool mustBeFalse(	const ExecutionState& es,
				ref<Expr> e,
				bool &result) override {
		CKSS_CHK(es);
	...
	bool getRange(	const ExecutionState& es,
			ref<Expr> query,
	...
	bool evaluate(	const ExecutionState& es,
			ref<Expr> e,
			Solver::Validity &result) override {
		CKSS_CHK(es);
		return base->evaluate(es, e, result);
	}

	bool mayBeTrue(	const ExecutionState& es,
			ref<Expr> e,
			bool &result) override {
		CKSS_CHK(es);
		return base->mayBeTrue(es, e, result);
	}

	bool mayBeFalse(const ExecutionState& es,
			ref<Expr> e,
			bool &result) override {
		CKSS_CHK(es);
		return base->mayBeFalse(es, e, result);
	}

	bool getValue(	const ExecutionState &es,
			ref<Expr> expr,
			ref<ConstantExpr> &result,
			ref<Expr> pred = 0) override {
		CKSS_CHK(es);
		return base->getValue(es, expr, result, pred);
	}

	bool getInitialValues(	const ExecutionState& es,
				Assignment& a) override {
		CKSS_CHK(es);
		return base->getInitialValues(es, a);
	}

	ref<Expr> toUnique(	const ExecutionState &es,
				const ref<Expr> &e) override {
		return (es.queryCost > maxTime)
			? nullptr
			: base->toUnique(es, e);
	}
private:
