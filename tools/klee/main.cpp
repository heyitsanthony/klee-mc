  ...
  if (!NoOutput) {
    ...
    if (WritePaths) {
      if (std::ostream* f = openTestFile("path", id)) {
        for(BranchTracker::iterator bit = state.branchDecisionsSequence.begin();
            bit != state.branchDecisionsSequence.end(); ++bit) {
...
void KleeHandler::getPathFiles(std::string path,
                               std::vector<std::string> &results) {
...
void KleeHandler::loadPathFile(std::string name,
                               Interpreter::ReplayPathType &buffer) {
